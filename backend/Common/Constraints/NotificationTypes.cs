namespace OTMS.Common.Constraints
{
    public class NotificationTypes
    {
        public const string TaskAssigned = "Task Assigned";
        public const string TaskUpdated = "Task Updated";
        public const string TaskDeadlineApproaching = "Task Deadline Approaching";
        public const string TaskReviewRequested = "Task Review Requested";
        public const string TaskReturnedForRework = "Task Returned For Rework";
        public const string TaskApprovedAndClosed = "Task Approved And Closed";
        public const string LeaveRequestCreated = "Leave Request Created";
        public const string EmergencyOverrideCreated = "Emergency Override Created";
        public const string ApprovalRequestPending = "Approval Request Pending";
        public const string ApprovalTierApproved = "Approval Tier Approved";
        public const string ApprovalRequestRejected = "Approval Request Rejected";
        public const string ApprovalRequestFinalApproved = "Approval Request Final Approved";
        public const string ApplicationSubmitted = "Application Submitted";
        public const string ApplicantStatusUpdated = "Applicant Status Updated";
        public const string OnboardingLinkSent = "Onboarding Link Sent";
        public const string AccountCreated = "Account Created";
        public const string ApplicationEmailVerified = "Application Email Verified";
    }
}
